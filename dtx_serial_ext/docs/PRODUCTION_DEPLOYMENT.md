...
### PHASE 4: System & Product Configuration (30 minutes)

#### Step 4.0: Enable Lot/Serial Number Tracking (REQUIRED!)

**⚠️ CRITICAL: Must enable this setting BEFORE configuring products!**

1. **Navigate to Inventory Settings:**
   - Go to: **Inventory** (main menu)
   - Click: **Configuration** > **Settings**

2. **Enable Traceability:**
   - Scroll to section: **Traceability**
   - Find option: **Lots & Serial Numbers**
   - ✅ **CHECK this option:**
     ```
     ☑ Lots & Serial Numbers
     Track products by lots or serial numbers
     ```
...
4. **Verify Setting Enabled:**
   - Refresh page
   - Option should remain **CHECKED** ✅
   - If unchecked, repeat steps 2-3

**What this enables:**
- ✅ Lot/Serial Number fields on products
- ✅ Serial tracking during stock moves
- ✅ Lot/Serial Number menu items
- ✅ Serial number assignment in receipts/deliveries

**⚠️ If you skip this step:**
- ❌ "Tracking" option won't appear on products
- ❌ Cannot assign serials during receipt
- ❌ Module features won't work!
...
│ Traceability                        │
├─────────────────────────────────────┤
│ ☑ Lots & Serial Numbers            │ ← Must be CHECKED!
│   Track products by lots or         │
│   serial numbers                    │
│                                     │
│ ☐ Expiration Dates                 │
│   Track expiration dates on lots    │
└─────────────────────────────────────┘
```
...
2. **Find your product:**
   - Search: Product name (e.g., "DTX Queue Display Device")
   - Or click: **Create** for new product

3. **Click: Edit** (if existing product)

---

##### B. Configure General Information Tab
...
│ General Information                         │
├─────────────────────────────────────────────┤
│ Product Name: [DTX Queue Display Device]   │
│ Can be Sold: ☑                             │
│ Can be Purchased: ☑                        │
│ Product Type: [Storable Product] ⬇         │ ← IMPORTANT!
│ Product Category: [Electronic Devices] ⬇   │ ← From Step 4.1
│ Sales Price: [1000.00]                      │
│ Cost: [800.00]                              │
│ Barcode: [optional]                         │
└─────────────────────────────────────────────┘
...
**Key fields:**
- **Product Type:** MUST be **"Storable Product"**
  - NOT "Consumable" or "Service"
  - Only storable products can have serial tracking
- **Product Category:** Select category from Step 4.1

---

##### C. Configure Inventory Tab ⭐ CRITICAL!

1. **Click: Inventory tab** (next to General Information)
...
│ Operations                                  │
│ ─────────────────────────────────────────   │
│ Tracking: [By Unique Serial Number] ⬇      │ ← MOST IMPORTANT!
│                                             │
│ ⚠️ Options:                                 │
│    • No Tracking                            │
│    • By Lots                                │
│    • By Unique Serial Number  ← Select this!│
│                                             │
│ Description for Receipts: [optional]        │
│ Description for Delivery Orders: [optional] │
│                                             │
│ ────────────────────────────────────────    │
│ Routes                                      │
│ ─────────────────────────────────────────   │
│ ☑ Buy                                       │ ← Check if purchasable
│ ☐ Manufacture                               │
│ ☑ Make To Order                             │ ← Optional
...
1. **"Tracking" field CANNOT be changed after stock moves exist!**
   - If product already has transactions → Create NEW product
   - Or ensure no stock moves exist before changing

2. **Must select "By Unique Serial Number":**
   - ✅ "By Unique Serial Number" = Each unit has unique serial (1:1)
   - ❌ "By Lots" = Multiple units share same lot number (N:1)
   - ❌ "No Tracking" = No serial tracking

3. **"Routes" configuration:**
   - ✅ Check "Buy" if you purchase this product
   - ✅ Check "Make To Order" for custom/special orders
   - Leave others unchecked unless needed

---

##### D. Configure Purchase Tab (If Purchasable)
...
│ ─────────────────────────────────────────   │
│ Control Policy: [On received quantities] ⬇  │ ← Recommended
└─────────────────────────────────────────────┘
```

**Recommended setting:**
- **Control Policy:** "On received quantities"
  - Ensures bills match received goods
  - Important for serial number tracking
...
2. **Verify configuration:**
   ...
   - Confirm: Tracking = "By Unique Serial Number" ✅

3. **Test immediately:**
   - Create test PO with this product
   - Validate receipt
   - Should see "Lot/Serial Number" field ✅

---

##### F. Repeat for All Products

**Create checklist:**

```markdown
Products to configure:
- [ ] DTX Queue Display Device
- [ ] DTX Ticket Printer
- [ ] DTX Counter Display
- [ ] ... (add all products)
```
...
- Configure similar products in one session
- Use consistent naming: "DTX [Product Type]"
- Same category for similar products
- Same routes for same product types

---

##### G. Verification Checklist

After configuring all products, verify:

- [ ] All products have **Product Type = "Storable Product"**
- [ ] All products have **Tracking = "By Unique Serial Number"**
- [ ] All products assigned to correct **Product Category**
- [ ] All products have **Routes** configured (Buy, etc.)
- [ ] No error messages when saving
- [ ] Test PO created successfully with one product

**If any check fails, review and fix before proceeding!**
