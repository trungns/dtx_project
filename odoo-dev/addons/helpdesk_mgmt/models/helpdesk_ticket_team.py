...
class HelpdeskTeam(models.Model):
    ...
    approval_stage_ids = fields.Many2many(
        comodel_name="helpdesk.ticket.stage",
        relation="helpdesk_team_approval_stage_rel",
        column1="team_id",
        column2="stage_id",
        string="Giai đoạn cần phê duyệt",
        help="Các giai đoạn cần có sự phê duyệt từ người phê duyệt",
        domain="['|', ('team_ids', '=', False), ('team_ids', '=', id)]",
        tracking=True,
    )

    @api.onchange('approval_stage_ids')
    def _onchange_approval_stage_ids(self):
        for team in self:
            if not team.approval_stage_ids:
                team.approver_id = False
            
    approver_id = fields.Many2one(
        comodel_name="res.users",
        string="Người phê duyệt",
        help="Người có quyền phê duyệt ticket ở giai đoạn cần phê duyệt",
        tracking=True
    )
    
    # Cờ động theo user hiện tại
    is_helpdesk_manager = fields.Boolean(
        string="Là Helpdesk Manager",
        compute="_compute_is_helpdesk_manager",
    )
    
    @api.depends_context("uid")
    def _compute_is_helpdesk_manager(self):
        is_mgr = bool(self.env.user.has_group("helpdesk_mgmt.group_helpdesk_manager"))
        for rec in self:
            rec.is_helpdesk_manager = is_mgr
    ...
    def default_get(self, fields_list):
        ...
        if 'is_helpdesk_manager' in fields_list:
            res['is_helpdesk_manager'] = self.env.user.has_group(
